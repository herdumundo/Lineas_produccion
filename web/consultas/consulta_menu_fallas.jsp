...
<%
	...
	fuente.setConexion(cn);
        ...
        try {
         ...
         ResultSet rs = fuente.obtenerDato("select distinct m.id,m.nro_carro,m.cantidad_actual, convert(char(10), v.InDate, 103) as fecha, "
            + "ar.U_TIPOHUEVO  "
            + "from "
            + " carros_asignados m with(nolock) , "
            + "zz_prueba_vimar.dbo.obtn v     with(nolock) "
            + "inner join zz_prueba_vimar.dbo.oitm ar   with(nolock) on v.itemcode=ar.itemcode "
            + "inner join zz_prueba_vimar.dbo.obbq b    with(nolock) on v.AbsEntry=b.SnBMDAbs and b.WhsCode='CEN002'  "
            + "where "
